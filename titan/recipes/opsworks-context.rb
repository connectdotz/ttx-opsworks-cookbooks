...
ruby_block "update hbase.host_name" do 
	block do
	
		layer_name = node[:titan][:opsworks][:hbase_master_layer]
		layer = defined?(node[:opsworks][:layers][layer_name]) == nil ? nil : node[:opsworks][:layers][layer_name]
		...
		node.override[:titan][:opsworks][:hbase_master_changed] = false
		instance = nil
		...
		if layer != nil
			...
			if (instance != nil)
				new_name = instance[:private_ip]
				if(new_name != node[:titan][:hbase][:host_name])
					node.override[:titan][:hbase][:host_name] = new_name
					Chef::Log.info("found updated hbase_master instance: #{instance.inspect}") 
					node.override[:titan][:opsworks][:hbase_master_changed] = true
				end
			end
		end

		if(instance == nil)
			Chef::Log.debug("will use default hbase.host_name") 
		end

		Chef::Log.info( "hbase_master_changed? #{node[:titan][:opsworks][:hbase_master_changed]}, host_name = #{node[:titan][:hbase][:host_name]}") 
